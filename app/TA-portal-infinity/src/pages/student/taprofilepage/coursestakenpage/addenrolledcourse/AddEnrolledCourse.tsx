...
export default function AddEnrollmentPage() {
    ...
    const onSelect = useCallback((sec: Section) => {
        ...
        setSelectedEnrollments(prev =>
            prev.some(e => e.section?.id === sid) ? prev : [...prev, { section: sec, status: 'ENROLLED' }]
        );
    }, []);
    ...
    const handleSave = async () => {
        ...
        const enrollmentResults = await Promise.all(
            toAdd.map(s =>
                fetchEnrollStudent({
                    studentId,
                    courseId: s.course?.id ?? s.section?.course?.id ?? -1,
                ...
                <div className="space-y-2">
                    {selectedEnrollments.map((e, idx) => {
                        const label = e.section
                            ? `${e.section.course?.deptCode} ${e.section.course?.courseNum} ${e.section?.section} – ${e.section.course?.name}`
